...
test.describe('ZenEngine Browser Tests', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto(`http://localhost:3000/bindings/web/test/test-page.html`);
    // Wait for WASM module to finish loading
    await page.waitForFunction(() => window.zenLoaded === true, { timeout: 10000 });
  });
  ...
  test('Credit analysis evaluation', async ({ page }) => {
    ...
    // Credit analysis uses function nodes which require QuickJS (disabled in WASM)
    // So we expect it to fail with a QuickJS error
    expect(result.success).toBe(false);
    expect(result.error).toContain('QuickJS');
  });
