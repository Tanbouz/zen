...
impl ZenEngine {
    ...
    pub async fn evaluate(&self, context: JsValue) -> Result<JsValue, JsValue> {
        ...
        let trace = context_val.get("trace").and_then(|v| v.as_bool()).unwrap_or(false);
        
        let options = EvaluationSerializedOptions {
            trace: if trace { EvaluationTraceKind::Default } else { EvaluationTraceKind::None },
            max_depth: 5,
        };

        // Use evaluate_serialized which properly converts Variable to Value
        let result = self.decision.evaluate_serialized(context_val.into(), options).await
